...
private:
    ...
    void StartIncast();
    std::map<Ptr<Node>, std::set<uint16_t>> m_usedPorts;
};
...
FatTreeTopology::StartIncastTraffic()
{
    ...
    // Schedule first incast
    Simulator::Schedule(Seconds(0.5), &FatTreeTopology::ScheduleNextIncast, this);
}
...
FatTreeTopology::GetRandomServerPair()
{
    ...
    NS_LOG_INFO("GetRandomServerPair() returning sender=" << sender << ", receiver=" << receiver 
...
//    }
    double interArrival = std::max(0.0001, m_exponentialRandom->GetValue(1.0 / lambda, 0.0)); // clamp to avoid huge gaps
    double nextTime = Simulator::Now().GetSeconds() + interArrival;

    NS_LOG_INFO("[BG] Now: " << Simulator::Now().GetSeconds()
                 << "s, InterArrival: " << interArrival
                 << "s, NextTime: " << nextTime
                 << "s, SIMULATION_TIME: " << SIMULATION_TIME);

    if (nextTime < SIMULATION_TIME)
    {
        Simulator::Schedule(Seconds(interArrival), &FatTreeTopology::StartBackgroundFlow, this);
    }
    else
    {
        NS_LOG_WARN("🚨 [BG] Not scheduling next flow. Would exceed simulation time.");
    }
...
FatTreeTopology::StartBackgroundFlow()
{
    ...
    NS_LOG_INFO("StartBackgroundFlow() called at time " << Simulator::Now().GetSeconds() << "s (" << Simulator::Now().GetMilliSeconds() << "ms)");
    ...
    NS_LOG_INFO("Selected sender " << senderIdx << " and receiver " << receiverIdx);
    ...
    Ipv4Address senderAddr = GetServerAddress(senderRack, senderPos);
    Ipv4Address receiverAddr = GetServerAddress(receiverRack, receiverPos);
    
    // Sample flow size
    uint32_t flowSize = SampleHadoopFlowSize();
    
    NS_LOG_INFO("Background flow: " << senderIdx << " -> " << receiverIdx << 
                " (" << flowSize << " bytes)");
    NS_LOG_INFO("Receiver address: " << receiverAddr << " (rack " << receiverRack << ", pos " << receiverPos << ")");
    ...
    // Create UDP sink on receiver
    // uint16_t port = 9000 + m_uniformRandom->GetInteger(0, 999);
    uint16_t port;
    do {
	    port = 9000 + m_uniformRandom->GetInteger(0, 999);
    } while (m_usedPorts[receiver].count(port) > 0);
    m_usedPorts[receiver].insert(port);

    NS_LOG_INFO("About to create UDP sink on receiver " << receiverIdx << " (rack " << receiverRack 
                << ", pos " << receiverPos << ") with port " << port << " at " << Simulator::Now().GetMilliSeconds() << "ms");
    
    PacketSinkHelper sinkHelper("ns3::UdpSocketFactory", InetSocketAddress(Ipv4Address::GetAny(), port));
    ApplicationContainer sinkApp = sinkHelper.Install(receiver);
    
    NS_LOG_INFO("UDP sink created successfully on receiver " << receiverIdx);
...
//    }
    double interArrival = std::max(0.0001, m_exponentialRandom->GetValue(1.0 / lambda, 0.0));
    double nextTime = Simulator::Now().GetSeconds() + interArrival;

    NS_LOG_INFO("[Incast] Now: " << Simulator::Now().GetSeconds()
                 << "s, InterArrival: " << interArrival
                 << "s, NextTime: " << nextTime
                 << "s, SIMULATION_TIME: " << SIMULATION_TIME);
    ...
    {
        NS_LOG_WARN("🚨 [Incast] Not scheduling next incast. Would exceed simulation time.");
    }
...
FatTreeTopology::StartIncast()
{
    ...
    NS_LOG_INFO("Incast receiver: " << receiverIdx << " (rack " << receiverRack << ", pos " << receiverPos << ")");
    NS_LOG_INFO("Receiver address: " << receiverAddr);
    ...
    // Create TCP sink on receiver
    // uint16_t port = 8000 + m_uniformRandom->GetInteger(0, 999);
    uint16_t port;
    do {
	    port = 8000 + m_uniformRandom->GetInteger(0, 999);
    } while (m_usedPorts[receiver].count(port) > 0);
    m_usedPorts[receiver].insert(port);
    ...
    NS_LOG_INFO("Started incast with " << numSenders << " senders to receiver " << receiverIdx);
...
{
    // Configure logging
    LogLevel logLevel = (LogLevel)(LOG_PREFIX_LEVEL | LOG_PREFIX_TIME | LOG_PREFIX_NODE | LOG_LEVEL_INFO);
    LogComponentEnable("FatTreeSim", logLevel);
    
    // Enable debug logging for address retrieval
    LogComponentEnable("FatTreeSim", LOG_LEVEL_DEBUG);
    ...
    Simulator::Run();
    std::cout << "Simulation finished at: " << Simulator::Now().GetSeconds() << " seconds." << std::endl; 
